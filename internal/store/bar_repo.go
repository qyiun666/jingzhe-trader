...
func (r *BarRepo) BatchInsert(bars []model.Bar) error {
	return batchInsert(r.db, barInsertSQL, "插入日线失败", len(bars), func(stmt *sqlx.Stmt, i int) error {
		b := bars[i]
		_, err := stmt.Exec(
...
func (r *BarRepo) GetBars(tsCode, startDate, endDate string) ([]model.Bar, error) {
	...
	var bars []model.Bar
	if err := selectList(r.db, query, &bars, "查询日线失败", tsCode, startDate, endDate); err != nil {
		return nil, err
	}
	return bars, nil
}
...
func (r *BarRepo) GetBarsByDate(tradeDate string) ([]model.Bar, error) {
	...
	var bars []model.Bar
	if err := selectList(r.db, query, &bars, "查询日线失败", tradeDate); err != nil {
		return nil, err
	}
	return bars, nil
}
