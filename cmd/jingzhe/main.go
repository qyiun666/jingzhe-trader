...
//	jingzhe db audit|rebuild-bar                 库结构清点 / daily_bar 重建为 WITHOUT ROWID
package main
...
func main() {
	...
	// 启动清点发现库结构与现役 schema 不一致时出声：建表只做加法，跨版本重写留下的
	// 废表/重复索引不会自己消失。这里只提示，不自动 DROP（删表不可逆）。
	// db 子命令自己会打印完整清点结果，跳过以免同一段话说两遍。
	if note := st.SchemaAuditNote(); note != "" && args[0] != "db" {
		fmt.Fprintf(os.Stderr, "⚠ 库结构清点发现差异（不自动处理，见 `jingzhe db audit`）:\n%s", note)
	}
